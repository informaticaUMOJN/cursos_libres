...
<?php }
	else
	{
		...
		$mbPermisoUsuario = fxPermisoUsuario("catClases", $mbAgregar, $mbModificar, $mbBorrar, $mbAnular);
		...
		if (isset($_POST["UMOJN"]))
            {
                fxBorrarAsignatura($_POST["UMOJN"]);
				fxAgregarBitacora($_SESSION["gsUsuario"], "UMO060A", $_POST["UMOJN"], "", "Borrar", "");
            }
		?>
    	<div class="container">
        	<div id="DivContenido">
				...
				<div class="row">
					<div class="col-md-12">
						...
						<table id="grid" class="table table-condensed table-hover table-striped" data-selection="true" data-multi-select="false" data-row-select="true" data-keep-selection="true">
                            <thead>
                                <tr>
                                    <th data-column-id="CLASES_REL" data-identifier="true" data-width="15%" data-align="left">Codigo de clase</th>
                                    <th data-column-id="NOMBRE_CURSO" data-width="30%" data-align="left">Curso</th>
                                    <th data-column-id="NOMBRE_MODULO" data-width="25%" data-align="left">Módulo</th>
                                    <th data-column-id="NOMBRE_CLASE" data-width="30%" data-align="left">Clase</th>
                                </tr>
                            </thead>
                            <tbody>
                            <?php
                                $mDatos = fxDevuelveClases(1);
                                
                                while ($mFila = $mDatos->fetch())
                                {
                                    echo "<tr>";
                                    echo "<td>" . $mFila["CLASES_REL"] . "</td>";
                                    echo "<td>" . $mFila["NOMBRE_CURSO"] . "</td>";
                                    echo "<td>" . $mFila["NOMBRE_MODULO"] . "</td>";
                                    echo "<td>" . $mFila["NOMBRE_CLASE"] . "</td>";
                                    echo "</tr>";
                                }  }
                            ?>
                            </tbody>
...
<script>
	...
	$(function() {
		...
		$("#append").on("click", function() {
			$.redirect("catClases.php", "POST");
		});

		$("#remove").on("click", function() {
			if ($.trim($("#grid").bootgrid("getSelectedRows")) != "")
			{
				var codClases = $.trim($("#grid").bootgrid("getSelectedRows"));
				$.redirect("gridClases.php", {UMOJN: codClases}, "POST");
			}
		});
		...
		$("#edit").on("click", function() {
			...
			{
				var codClases = $.trim($("#grid").bootgrid("getSelectedRows"));
				$.redirect("catClases.php", {UMOJN: codClases}, "POST");
			}
		});
