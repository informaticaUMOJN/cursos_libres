...
$mbPermisoUsuario = fxPermisoUsuario("procPagosCL");
...
if (isset($_POST["txtPago"])) {
    ...
    foreach ($gridDetalle as $mRegistro) {
        ...
        if ($abonado > 0 || $descuento > 0) {
            // Cambio aquí: fxGuardarDetPago ahora apunta a UMO142A
            fxGuardarDetPago($cobro, $matricula, $abonado, $descuento, $msCodigo, 'UMO142A');
        }
    }

    echo '<meta http-equiv="Refresh" content="0;url=gridPagoCL.php"/>';
    exit;
    ...
    <div id="DivContenido">
        ...
        <div class="row">
            <div class="col-xs-12 offset-sm-none col-md-12 offset-md-1">
               <form id="procPagosCL" name="procPagosCL" action="procPagosCL.php" method="POST" onsubmit="return validarRecibo();">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-12 col-md-6">
                            <?php
                                $disabled = $msEstudiante == "" ? "disabled" : "";
                                ...
                                while ($mFila = $mDatos->fetch()) {
                                    ...
                                    $msTexto = trim($mFila["NOMBRES_200"]) . " " . trim($mFila["APELLIDOS_200"]);
                                    $selected = ($msEstudiante == $msValor) ? "selected" : "";
                        ...
                        <div class="col-sm-auto col-md-10">
                            <div id="dvPG">
                                <table id="dgPG" class="easyui-datagrid table" data-options="iconCls:'icon-edit', fitColumns:true, height:'200px', width:'100%'">
                                    ...
                                    <tbody>
                                        <?php
                                            $mDatos = fxObtenerRealizarPago($msMatriculaCodigo, 'UMO132A');
                                            while ($mFila = $mDatos->fetch()) {
                                                $moneda = $mFila["MONEDA_132"];
                                                $adeudado = $mFila["ADEUDADO_132"] ?? 0;
                                                $descripcion = $mFila["DESC_130"];
                                                $abonado = $mFila["ABONADO_132"] ?? 0;
                                                $descuento = $mFila["DESCUENTO_132"] ?? 0;
                                                if ($abonado > 0) {
                                                    $descuento = 0;
                                                    $abonado = 0;
                                                }
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const form = document.getElementById('procPagosCL');
